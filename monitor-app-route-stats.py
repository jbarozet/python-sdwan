...
def approute_report(hub_list_file):
    ...
    try:
        try:
            ...
            raise ValueError(
                "Incorrect start data format, please enter in YYYY-MM-DD")
        try:
            end_date = input("Please enter end date(YYYY-MM-DD): ")
            time.strptime(end_date, '%Y-%m-%d')
        except ValueError:
            raise ValueError(
                "Incorrect end data format, please enter in YYYY-MM-DD")
        ...
        if response.status_code == 200:
            ...
            for item in temp:
                if item["personality"] == "vedge":
                    device_inv[item["system-ip"]] = [
                        {'hostname': item["host-name"]}, {'siteid': item["site-id"]}]
        else:
        ...
        # open excel file
        writer = ExcelWriter('Tunnel Statistics %s to %s.xlsx' %
                             (start_date, end_date))

        PDT = pytz.timezone('America/Los_Angeles')
        file_name = open("Tunnel Statistics %s to %s.csv" %
        ...
        for hub in config["hub_routers"]:
            ...
            if response.status_code == 200:
                app_route_stats = response.json()["data"]
                app_route_stats_headers = ["Date (PDT)", "Hub", "Hub Siteid", "Spoke", "Spoke Siteid",
                                           "Tunnel name", "vQoE score", "Latency", "Loss percentage", "Jitter"]
                date_list = list()
                hub_list = list()
                ...
                for item in app_route_stats:
                    ...
                    temp_time = datetime.datetime.utcfromtimestamp(
                        item['entry_time']/1000.)
                    temp_time = pytz.UTC.localize(
                        temp_time).astimezone(PDT).strftime('%m/%d/%Y')
                    #date_list.append(time.strftime('%m/%d/%Y',  time.gmtime(item['entry_time']/1000.)))

                    tr = [temp_time, device_inv[item['local_system_ip']][0]['hostname'], device_inv[item['local_system_ip']][1]['siteid'], device_inv[item['remote_system_ip']][0]
                          ['hostname'], device_inv[item['remote_system_ip']][1]['siteid'], item['name'], item['vqoe_score'], item['latency'], item['loss_percentage'], item['jitter']]
                    table.append(tr)

                    date_list.append(temp_time)
                    hub_list.append(
                        device_inv[item['local_system_ip']][0]['hostname'])
                    hub_siteid_list.append(
                        device_inv[item['local_system_ip']][1]['siteid'])
                    spoke_list.append(
                        device_inv[item['remote_system_ip']][0]['hostname'])
                    spoke_siteid_list.append(
                        device_inv[item['remote_system_ip']][1]['siteid'])
                ...
                csv_content = csv_content + \
                    tabulate.tabulate(
                        table, app_route_stats_headers, tablefmt="csv") + "\n"
                excel_content = dict()
                ...
                df.to_excel(
                    writer, device_inv[hub["system_ip"]][0]['hostname'], index=False)
